...
---------------------------------------------------------------------
-- Pedido 10883 y 10815 están empatados, si necesitamos rownum deterministra, incluir orderid in order by clauses

SELECT custid, orderid, val,
  ROW_NUMBER() OVER(ORDER BY val) AS rownum,
  RANK()       OVER(ORDER BY val) AS rnk,
  DENSE_RANK() OVER(ORDER BY val) AS densernk,
  NTILE(10)   OVER(ORDER BY val) AS ntile10 --10 buckets
FROM Sales.OrderValues
-- where orderid = 11057
-- order by orderid;
...
---------------------------------------------------------------------
-- Únicamente calculamos en base a una fila y no a un conjunto.
