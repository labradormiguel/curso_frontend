...
-- Empleados que han recogido pedidos para cliente 1 y cliente 2, en ese orden

select distinct s1.empid
from 
	(select * from sales.orders where custid = 1 ) s1 
cross join ( select * from sales.orders where custid = 2) s2 
where s1.empid = s2.empid and s1.orderdate < s2.orderdate